...
export default async function List({ children, data, title, subtitle, icon, documents }) {
  ...
  const BytesPerKilobytes = (byte) => {
    const Kilobytes = byte / 1024
  ...
  return posts ? (
    <main className="list">
      ...
      <section className="list__posts">
        {posts.map((post) => (
          <Link href={post.file?.url ? post.file.url : '#'} key={post.id}>
              ...
              <div className="list__span">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: documents
                      ? `${post.file?.fileName} ${BytesPerKilobytes(post.file?.size)}KB`
